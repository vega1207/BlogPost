...
class BlogPost(db.Model):
  ...
  __searchable__ = ['title', 'content','author']  # these fields will be indexed by whoosh
    ...
    if 'loggedin' in session:
        ...
        for account in accounts:
            ...
            if request.method =='POST':
              ...
              return render_template('profileList.html',posts=all_posts.items,pages=page,pagination = all_posts)
    # User is not loggedin redirect to login page
    return redirect(url_for('login'))

@app.route('/profile/download/<int:id>')
def download(id):
  if 'loggedin' in session:
...
@app.route('/profile/view/<int:id>',methods= ['GET','POST'])
def view(id):
  if 'loggedin' in session:
    post = BlogPost.query.get_or_404(id)
    if request.method =='POST':
...
def search():
  if 'loggedin' in session:
    ...
    print(keyword)
    results = BlogPost.query.msearch(keyword,fields=['title','content','author'],limit=20).all()
    return render_template('profileList.html',posts=results)
  else:
    return redirect(url_for('login')) 
